# SteaMidra User Guide - How to Use
...
### Process a .lua file
The main functionality of SteaMidra. It goes through 8 steps:

**1. Input Methods**
- Add a .lua file: Manually import a .lua file that you own
- Choose from saved .lua files: Every .lua file you add gets saved by SteaMidra, you can find them again through here (usually to update games)
...
There are two types of DLC. Ones that have a depot and ones that don't. SteaMidra calls them DOWNLOAD REQUIRED and PRE-INSTALLED respectively.

- For DOWNLOAD REQUIRED, you'll need to find a .lua that contains keys for that DLC
- For PRE-INSTALLED, all you need to do is allow SteaMidra to add them to your AppList folder
...
### Check for updates
You can update SteaMidra using this. It's a bit funky though.

### Install/Uninstall Context Menu
You can immediately access SteaMidra through right-clicking a .lua/.zip file. This just launches SteaMidra immediately into the Process a .lua file section, along with the file you right-clicked.

### Settings
You can change settings set by SteaMidra here. They usually get set automatically as you use SteaMidra.

---

## Main Features

### 1. Process .lua Files (Standard SteaMidra feature)
...
6. SteaMidra will automatically:
   - Add to AppList
   - Check for DLC
   - Add decryption keys
   - Write ACF file
...
- Stored in SteaMidra settings (encrypted)
...
### "Permission denied"
- Run SteaMidra as administrator
...
### Main Menu Options:
- **Process a .lua file** - Standard SteaMidra feature
...
### File Locations:
- **Credentials**: Stored in SteaMidra settings (encrypted)
- **Downloads**: Temporary folder (auto-deleted)
- **Extracted files**: Game installation folder
- **Logs**: `debug.log` in SteaMidra folder
...
**SteaMidra now has two main features**:
...
Both work automatically with minimal user input. Just select what you want and SteaMidra handles the rest!
